...
jest.mock("../services/api", () => ({
  ...
  configureLightingSystem: (...args: any[]) =>
    mockApiCalls.configureLightingSystem(...args),
}));
...
jest.mock("@ionic/react", () => ({
  ...
  IonItem: ({ children, ...props }: any) => (
    <div data-testid="ion-item" {...props}>
      {children}
    </div>
  ),
  IonLabel: ({ children }: any) => (
    <label data-testid="ion-label">{children}</label>
  ),
  IonInput: ({ onIonInput, placeholder, ...props }: any) => (
    <input
      ...
      onChange={(e) =>
        onIonInput && onIonInput({ detail: { value: e.target.value } })
      }
      {...props}
    />
  ),
  ...
  IonSelect: ({ children, onSelectionChange, placeholder }: any) => (
    <select
      ...
      onChange={(e) =>
        onSelectionChange &&
        onSelectionChange({ detail: { value: e.target.value } })
      }
    >
    ...
    isOpen ? (
      <div data-testid="ion-alert">
        ...
        {buttons?.map((btn: any, idx: number) => (
          <button key={idx} onClick={btn.handler}>
            {btn.text}
          </button>
        ))}
      </div>
...
describe("DeviceSetupFlow Integration", () => {
  ...
  describe("Device Claiming Flow", () => {
    ...
    it("should handle invalid pairing code", async () => {
      // Mock API error
      mockApiCalls.claimDevice.mockRejectedValue(
  ...
  describe("Lighting System Configuration", () => {
    ...
    it("should configure Nanoleaf lighting system", async () => {
      ...
      render(
        <DeviceSetupFlow initialStep="lighting-config" deviceId="device-123" />
      );
      ...
      await waitFor(() => {
        expect(mockApiCalls.configureLightingSystem).toHaveBeenCalledWith(
          "device-123",
          {
            lightingSystemType: "nanoleaf",
    ...
    it("should handle Nanoleaf authentication flow", async () => {
      const mockUserNotificationService =
        UserNotificationService as jest.MockedClass<
      ...
      const notificationCallback =
        mockInstance.onAuthenticationNotification.mock.calls[0][1];
